...
# J Neuroscience 2013
#
# *HybridCaburst_stochSK.py : A hybrid calcium burst model with stochastic SK 
...
# USAGE
#
# $ python HybridCaburst_stochSK.py *mesh* *root* *iter_n* 
#  
...
#
# E.g: python HybridCaburst_stochSK.py Cylinder2_dia2um_L10um_outer0_3um_0.3shell_0.3size_19156tets_adaptive.inp ~/stochcasims/ 1
#
#
# OUTPUT 
#
# In (root)/data/HybridCaburst_stochSK/(mesh)/(iter_n+time) directory 
...
SKCAC1 = smodel.SReac('SKCAC1', ssys_stoch, slhs = [SK_C1], ilhs = [Ca_stoch], srhs = [SK_C2], kcst = dirc2_t)
SKCAC2 = smodel.SReac('SKCAC2', ssys_stoch, slhs = [SK_C2], ilhs = [Ca_stoch], srhs = [SK_C3], kcst = dirc3_t)
SKCAC3 = smodel.SReac('SKCAC3', ssys_stoch, slhs = [SK_C3], ilhs = [Ca_stoch], srhs = [SK_C4], kcst = dirc4_t)

SKC1 = smodel.SReac('SKC1', ssys_stoch, slhs = [SK_C2], srhs = [SK_C1], irhs = [Ca_stoch], kcst = invc1_t)
SKC2 = smodel.SReac('SKC2', ssys_stoch, slhs = [SK_C3], srhs = [SK_C2], irhs = [Ca_stoch], kcst = invc2_t)
SKC3 = smodel.SReac('SKC3', ssys_stoch, slhs = [SK_C4], srhs = [SK_C3], irhs = [Ca_stoch], kcst = invc3_t)

SKC3O1 = smodel.SReac('SKC3O1', ssys_stoch, slhs = [SK_C3], srhs = [SK_O1], kcst = diro1_t)
SKC4O2 = smodel.SReac('SKC4O2', ssys_stoch, slhs = [SK_C4], srhs = [SK_O2], kcst = diro2_t)

SKO1C3 = smodel.SReac('SKO1C3', ssys_stoch, slhs = [SK_O1], srhs = [SK_C3], kcst = invo1_t)
SKO2C4 = smodel.SReac('SKO2C4', ssys_stoch, slhs = [SK_O2], srhs = [SK_C4], kcst = invo2_t)

OC1_SK = smodel.OhmicCurr('OC1_SK', ssys_stoch, chanstate = SK_O1, erev = SK_rev, g = SK_G )
OC2_SK = smodel.OhmicCurr('OC2_SK', ssys_stoch, chanstate = SK_O2, erev = SK_rev, g = SK_G )
...
# SK
sim_stoch.setPatchCount('memb_stoch', 'SK_C1' , round(SK_ro*surfarea*SK_C1_p))
sim_stoch.setPatchCount('memb_stoch', 'SK_C2' , round(SK_ro*surfarea*SK_C2_p))
sim_stoch.setPatchCount('memb_stoch', 'SK_C3' , round(SK_ro*surfarea*SK_C3_p))
sim_stoch.setPatchCount('memb_stoch', 'SK_C4' , round(SK_ro*surfarea*SK_C4_p))

sim_stoch.setPatchCount('memb_stoch', 'SK_O1' , round(SK_ro*surfarea*SK_O1_p))
sim_stoch.setPatchCount('memb_stoch', 'SK_O2' , round(SK_ro*surfarea*SK_O2_p))
...
try: os.mkdir(root+'data/' +  'HybridCaburst_stochSK')
except: pass
try: os.mkdir(root+'data/' +  'HybridCaburst_stochSK/'+meshfile_ab)
except: pass 

os.mkdir(root+'data/' +  'HybridCaburst_stochSK/'+meshfile_ab+'/'+iter_n+'__'+dc )


datfile =  open(root+'data/' +  'HybridCaburst_stochSK/'+meshfile_ab+'/'+iter_n+'__'+dc + '/currents.dat', 'w')
datfile2 = open(root+'data/' +  'HybridCaburst_stochSK/'+meshfile_ab+'/'+iter_n+'__'+dc + '/voltage.dat', 'w')
datfile3 = open(root+'data/' +  'HybridCaburst_stochSK/'+meshfile_ab+'/'+iter_n+'__'+dc + '/calcium.dat', 'w')
...
for l in range(NTIMEPOINTS):
    ...
    for m in submemb_tets:
        ...
        sim_det.setPatchSReacK('memb_det','CaPm0m1', 1.0e3 *3.* alpha_cap(V*1.0e3)*Qt)
        sim_det.setPatchSReacK('memb_det','CaPm1m2', 1.0e3 *2.* alpha_cap(V*1.0e3)*Qt)
        sim_det.setPatchSReacK('memb_det','CaPm2m3', 1.0e3 *1.* alpha_cap(V*1.0e3)*Qt)
        sim_det.setPatchSReacK('memb_det','CaPm3m2', 1.0e3 *3.* beta_cap(V*1.0e3)*Qt)
        sim_det.setPatchSReacK('memb_det','CaPm2m1', 1.0e3 *2.* beta_cap(V*1.0e3)*Qt)
        sim_det.setPatchSReacK('memb_det','CaPm1m0', 1.0e3 *1.* beta_cap(V*1.0e3)*Qt)
    ...
    for m in submemb_tets:
        ...
        cur_SK = sim_stoch.getTriOhmicI(ctriID,'OC1_SK')+ sim_stoch.getTriOhmicI(ctriID,'OC2_SK')
        sim_stoch.setTriIClamp(ctriID, cur_CaP+cur_CaT+cur_BK+cur_L)
