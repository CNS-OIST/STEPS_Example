...
with mdl_stoch:
    ...
    with ssys_stoch:
        ...
        if cyl160:
            OC_CaP = GHKCurr.Create(CaPchan[CaP_m3], Ca_stoch, CaP_P, virtual_oconc=Ca_oconc, computeflux=True)
        else:
            OC_CaP = GHKCurr.Create(CaPchan[CaP_m3], Ca_stoch, CaP_P, computeflux=True)
...
with mdl_det:
    ...
    iCBsf, iCBsCa, iCBCaf, iCBCaCa, CBsf, CBsCa, CBCaf, CBCaCa, PV, PVMg, PVCa, Mg, Pump, CaPump,\
        CaT_m0h0, CaT_m0h1, CaT_m1h0, CaT_m1h1, CaT_m2h0, CaT_m2h1, SK_C1, SK_C2, SK_C3, SK_C4, \
        SK_O1, SK_O2, BK_C0, BK_C1, BK_C2, BK_C3, BK_C4, BK_O0, BK_O1, BK_O2, BK_O3, \
        BK_O4 = Species.Create()
...
with mesh_stoch as mesh:
    ...
    if cyl160:
        ...
        outer_stoch = TetComp.Create(outer_tets.indices)
        memb_stoch = TetPatch.Create(memb_tris, cyto_stoch, outer_stoch, ssys_stoch)
...
sim_stoch.memb_stoch.CaPchan[CaP_m0].Count = round(CaP_ro*surfarea*CaP_m0_p)
sim_stoch.memb_stoch.CaPchan[CaP_m1].Count = round(CaP_ro*surfarea*CaP_m1_p)
sim_stoch.memb_stoch.CaPchan[CaP_m2].Count = round(CaP_ro*surfarea*CaP_m2_p)
sim_stoch.memb_stoch.CaPchan[CaP_m3].Count = round(CaP_ro*surfarea*CaP_m3_p)
...
runPath = os.path.join(root, 'data/HybridCaburst_stochCaP/', meshfile_ab, f'{iter_n}__{dc}')
...
for l in range(NTIMEPOINTS):
    ...
    currs_CaP = np.array(sim_stoch.TRIS(stris).OC_CaP.I)
    ...
    # Update sim stoch
    sim_stoch.TETS(stets).Ca_stoch.Count = sim_det.TETS(stets).Ca_det.Count - (currs_CaT) * TIMECONVERTER / (2 * E_CHARGE)
    sim_stoch.TRIS(stris).IClamp = currs_BK + currs_CaT + currs_SK + currs_L
