...
with mdl_det:
    ...
    iCBsf, iCBsCa, iCBCaf, iCBCaCa, CBsf, CBsCa, CBCaf, CBCaCa, PV, PVMg, PVCa, Mg, Pump, CaPump,\
        CaT_m0h0, CaT_m0h1, CaT_m1h0, CaT_m1h1, CaT_m2h0, CaT_m2h1, BK_C0, BK_C1, BK_C2, BK_C3, \
        BK_C4, BK_O0, BK_O1, BK_O2, BK_O3, BK_O4, CaP_m0, CaP_m1, CaP_m2, CaP_m3 = Species.Create()
...
# SK
sim_stoch.memb_stoch.SKchan[SK_C1].Count = round(SK_ro*surfarea*SK_C1_p)
sim_stoch.memb_stoch.SKchan[SK_C2].Count = round(SK_ro*surfarea*SK_C2_p)
sim_stoch.memb_stoch.SKchan[SK_C3].Count = round(SK_ro*surfarea*SK_C3_p)
sim_stoch.memb_stoch.SKchan[SK_C4].Count = round(SK_ro*surfarea*SK_C4_p)

sim_stoch.memb_stoch.SKchan[SK_O1].Count = round(SK_ro*surfarea*SK_O1_p)
sim_stoch.memb_stoch.SKchan[SK_O2].Count = round(SK_ro*surfarea*SK_O2_p)
...
runPath = os.path.join(root, 'data/HybridCaburst_stochSK/', meshfile_ab, f'{iter_n}__{dc}')
...
for l in range(NTIMEPOINTS):
    ...
    currs_SK = np.array(sim_stoch.TRIS(stris).OC_SK.I)
    ...
    # Update sim stoch
    sim_stoch.TETS(stets).Ca_stoch.Count = sim_det.TETS(stets).Ca_det.Count - (currs_CaT + currs_CaP) * TIMECONVERTER / (2 * E_CHARGE)
    sim_stoch.TRIS(stris).IClamp = currs_BK + currs_CaT + currs_CaP + currs_L
