...
runPath = os.path.join(root, 'data/StochasticCaburst_dendrite/', meshfile_ab, f'{iter_n}__{dc}')
...
for l in range(NTIMEPOINTS):
    ...
    for i in range(len(Length)):
        ...
        for tri in Compborder_triIDs[i]: 
            sim_stoch.TRI(tri).IClamp = (tcur_CaP+tcur_CaT+tcur_BK+tcur_SK)*AreaRatio/nmtris
