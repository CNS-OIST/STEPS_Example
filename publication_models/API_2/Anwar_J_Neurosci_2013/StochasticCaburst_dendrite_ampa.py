...
with mdl_WM:
    ...
    # Pump
    Pump, CaPump, iCBsf, iCBsCa, iCBCaf, iCBCaCa, CBsf, CBsCa, CBCaf, CBCaCa, PV, PVMg, PVCa, Mg, CaP_m0, CaP_m1, CaP_m2, CaP_m3, CaT_m0h0, CaT_m0h1, CaT_m1h0, CaT_m1h1, CaT_m2h0, CaT_m2h1, BK_C0, BK_C1, BK_C2, BK_C3, BK_C4, BK_O0, BK_O1, BK_O2, BK_O3, BK_O4, SK_C1, SK_C2, SK_C3, SK_C4, SK_O1, SK_O2, AMPA_C, AMPA_C1, AMPA_C2, AMPA_D1, AMPA_D2, AMPA_O = Species.Create()
...
with ssys_chans, mdl_stoch:
    ...
    AMPA_C.s <r['AMPACC1']> AMPA_C1.s ; r['AMPACC1'].setRates(0.0, ru1)
    AMPA_C1.s <r['AMPAC1C2']> AMPA_C2.s ; r['AMPAC1C2'].setRates(0.0, ru2)
    AMPA_C2.s <r['AMPAC2O']> AMPA_O.s ; r['AMPAC2O'].setRates(ro, rc)
    
    
    AMPA_C1.s <r['AMPAD1']> AMPA_D1.s ; r['AMPAD1'].setRates(rd, rr)
    AMPA_C2.s <r['AMPAD2']> AMPA_D2.s ; r['AMPAD2'].setRates(rd, rr)
with mdl_stoch:
    ...
    for j in range(int(Nannulis[i])):
        ...
        if j==0:
            ...
            if i>1:
                ...
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_C.Count = round(AMPA_receptors)
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_C1.Count = 0
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_C2.Count = 0
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_O.Count = 0
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_D1.Count = 0
                getattr(sim_WM, 'rings'+str(i)+str(j)).AMPA_D2.Count = 0
...
try: os.mkdir(root+'data/' +  'StochasticCaburst_dendrite_ampa')
except: pass
try: os.mkdir(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab)
except: pass 

os.mkdir(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc )

datfile =  open(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc + '/currents.dat', 'w')
datfile2 = open(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc + '/voltage.dat', 'w')
datfile3 = open(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc + '/calcium.dat', 'w')
...
for l in range(NTIMEPOINTS):
    ...
    if TIMECONVERTER*l in cp_times:
        ...
        sim_stoch.checkpoint(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc + '/checkpoint__sim_stoch__'+  str(TIMECONVERTER*l))
        # WARNING: Using a variable name that is reserved (['checkpoint']).
        sim_WM.checkpoint(root+'data/' +  'StochasticCaburst_dendrite_ampa/'+meshfile_ab+'/'+iter_n+'__'+dc + '/checkpoint__sim_WM__'+  str(TIMECONVERTER*l))
        datfile.flush()
        datfile2.flush()
        datfile3.flush()
    ...
    for i in range(len(Length)):
        ...
        if i>1:    
            ...
            getattr(sim_WM, 'rings'+str(i)+str(0)).AMPACC1['fwd'].K = 1.0e-3 *rb*Glut[l]
            getattr(sim_WM, 'rings'+str(i)+str(0)).AMPAC1C2['fwd'].K = 1.0e-3 *rb*Glut[l]
    ...
    for i in range(len(Length)):
        ...
        if i>1:
            ...
            tcur_AMPA = tcur_AMPA_sc*getattr(sim_WM, 'rings'+str(i)+str(0)).AMPA_O.Count
        ...
        for tri in Compborder_triIDs[i]: sim_stoch.setTriIClamp(tri, (((tcur_CaP+tcur_CaT+tcur_BK+tcur_SK+tcur_AMPA)*AreaRatio)/nmtris))
